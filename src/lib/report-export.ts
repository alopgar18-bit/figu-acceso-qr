...
export async function exportReportPDF(data: ReportData, opts: { sessionId?: string; perms?: VisibilityPermissions } = {}) {
  ...
  autoTable(doc, {
    ...
    body: [
      ["Solicitudes (personas)", String(data.totals.personasSolicitudes)],
      ["Aprobados (personas)", String(data.totals.personasAprobados)],
      ["Confirmados (personas)", String(data.totals.personasConfirmadas)],
      ...
      ["No presentados (personas)", String(data.totals.personasNoPresentados)],
      ["Cancelados (personas)", String(data.totals.personasCancelados)],
      ["Lista de espera (personas)", String(data.totals.personasListaEspera)],
...
export async function exportReportDetailExcel(data: ReportData, opts: { sessionId?: string; perms?: VisibilityPermissions } = {}) {
  ...
  const resumen: (string | number)[][] = [
    ...
    [],
    ["MÉTRICA", "VALOR"],
    ["Solicitudes (titulares)", t.solicitudes],
    ["Solicitudes (personas: titular + acompañantes)", t.personasSolicitudes],
    ["Pendientes de revisión", t.pendientes],
    ["Aprobados (titulares)", t.aprobados],
    ["Aprobados (personas)", t.personasAprobados],
    ["Rechazados (titulares)", t.rechazados],
    ["Rechazados (personas)", t.personasRechazados],
    ["Lista de espera (titulares)", t.listaEspera],
    ["Lista de espera (personas)", t.personasListaEspera],
    ["Confirmados (titulares)", t.confirmados],
    ["Confirmados (personas: titular + acompañantes)", t.personasConfirmadas],
    ["Cancelados (titulares)", t.cancelados],
    ["Cancelados (personas)", t.personasCancelados],
    ...
    ["No presentados (titulares)", t.noPresentados],
    ["No presentados (personas)", t.personasNoPresentados],
    ["Ratio asistencia % (sobre personas confirmadas)", t.personasConfirmadas ? Math.round((t.checkins / t.personasConfirmadas) * 100) : 0],
    [],
    ["INCIDENCIAS", ""],
    ...
    [],
    ["DESGLOSE POR SESIÓN", ""],
    ["Sesión", "Aforo", "Solic. (pers.)", "Aprob. (pers.)", "Conf. (titul.)", "Conf. (pers.)", "Asist.", "QR", "Manual", "Walk-in", "No-show (pers.)", "Inc.", "Ocup. %"],
    ...data.sessions.map((s) => [
      s.name, s.capacity, s.stats.personasSolicitudes, s.stats.personasAprobados, s.stats.confirmados,
      s.stats.personasConfirmadas, s.stats.checkins, s.stats.checkinsQr, s.stats.checkinsManual,
      s.stats.checkinsViaIncidencia, s.stats.personasNoPresentados, s.stats.incidencias,
      s.capacity ? Math.round((s.stats.personasConfirmadas / s.capacity) * 100) : 0,
    ]),
  ];
