...
import { Armchair, FileSpreadsheet, Loader2 } from "lucide-react";
...
function rowsFromRecords(records: Array<Record<string, unknown>>): SeatRow[] {
  ...
  const kTitular = find(["solicitante (titular)", "solicitante titular", "solicitante", "titular"]);
  ...
  for (const rec of records) {
    ...
    if (rawRol.startsWith("acomp") || rawRol === "companion") tipo = "acompanante";
      ...
      rawRol.startsWith("titu") ||
      rawRol.startsWith("solic") ||
      rawRol === "main" ||
      rawRol === "principal"
    ) {
      tipo = "titular";
...
async function parseExcel(file: File): Promise<SeatRow[]> {
  ...
  const preferred =
    wb.SheetNames.find((n) => normH(n) === "detalle") ??
          ...
          <div>
            ...
            <label className="mt-1 flex items-center gap-3 border-2 border-dashed rounded-md p-4 cursor-pointer hover:bg-muted/50 transition">
              ...
              <div className="flex-1 text-sm">
                {fileRows ? (
                  <span>
                    <strong>{fileName}</strong> — {fileRows.length} filas listas
                  </span>
                ) : (
                  <span className="text-muted-foreground">
                    Selecciona el Excel del informe o un CSV
                  </span>
                )}
              </div>
              {fileRows && (
